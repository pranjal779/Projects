# Local values for VPC Perring Task for Day 15
...
locals {
  ...
  secondary_user_data = <<-EOF
        ...
        systemctl start apache2
        systemctl enable apache2
        echo "<h1>Secondary VPC Instance - ${var.secondary}</h2" > /var/www/html/index.html
        echo "<p>Private IP: $(hostname -I)</p>" >> /var/www/html/index.html
    EOF
  ...
  third_user_data = <<-EOF
        ...
        systemctl start apache2
        systemctl enable apache2
        echo "<h1>Secondary VPC Instance - ${var.third}</h2" > /var/www/html/index.html
        echo "<p>Private IP: $(hostname -I)</p>" >> /var/www/html/index.html
    EOF
}
